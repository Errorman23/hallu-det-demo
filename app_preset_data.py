...
def get_page_header():
    ...
    <div style="margin-bottom: 20px;">
        <h1 style="margin: 0;">Hallucination Detection by Consistency Check</h1>
        <p style="margin: 5px 0 0 0; color: gray; font-size: 14px;">Detecting factual inconsistencies in LLM responses using temperature variation</p>
    </div>
    """
    return header_html
...
async def async_chat_complete(cfg: LLMConfig, messages: List[Dict], temperature: Optional[float] = None, seed: Optional[int] = None, retry_count: int = 0) -> str:
    ...
    try:
        ...
        if cfg.provider == "openai" and cfg.model == "gpt-5-mini":
            ...
            kwargs["max_completion_tokens"] = 2000  # Increased for GPT-5-mini
        ...
        st.error(f"LLM Error (attempt {retry_count + 1}): {error_msg}")
        return ""
...
"""
    ...
    try:
        ...
        cfg = LLMConfig(provider="openai", model="gpt-5-mini", max_tokens=10)
...
# -------------------------------
DOMAIN_PACKS = {
    "Singapore Insurance": "domain_packs/sg_insurance.json"
}

def load_domain_pack(path: str):
    with open(path, "r", encoding="utf-8") as f:
        return json.load(f)

def gate_question(q: str) -> Tuple[bool, str]:
    """Gate non-grounded prompts"""
    ql = q.strip().lower()
    bad_keywords = ["write a poem", "creative", "story", "imagine", "opinion", 
                   "should i", "feel", "believe", "forecast", "predict"]
    if any(x in ql for x in bad_keywords):
        return False, "Only factual questions are supported"
    if len(ql) < 8 or not q.endswith("?"):
        return False, "Please provide a proper question"
    return True, "OK"
...
def main():
    ...
    with st.sidebar:
        st.subheader("1) Domain & Questions")
        domain_name = st.selectbox("Domain pack", list(DOMAIN_PACKS.keys()))
        pack = load_domain_pack(DOMAIN_PACKS[domain_name])
        st.write(f"**{len(pack['questions'])}** questions loaded")
        
        if st.checkbox("Show questions", value=False):
            for i, q in enumerate(pack["questions"], 1):
                st.markdown(f"**Q{i}.** {q['text']}")  # Show full question
    ...
    cols = st.columns(3)
    with cols[0]:
        q_count = st.slider("Questions to process", 1, min(10, len(pack["questions"])), min(10, len(pack["questions"])), 1)
    with cols[2]:
        run_btn = st.button("🚀 Run Detection", type="primary")

    selected_qs = [q["text"] for q in pack["questions"][:q_count]]

    if run_btn:
        # Validate questions
        valid_qs = []
        for q in selected_qs:
            ok, why = gate_question(q)
            if ok:
                valid_qs.append(q)
            else:
                st.warning(f"Skipped: {q} - {why}")  # Show full question
    ...
    if st.session_state.detection_results:
        ...
        for r in results:
            ...
            with st.expander(f"{score_emoji} {r['question']} (Score: {r['overall_score']:.2f})", expanded=False):  # Full question
        ...
        st.download_button(
            ...
            json_str,
            file_name=f"hallucination_detection_{selected_model.replace('/', '_')}_{int(time.time())}.json",
    ...
    with st.expander("ℹ️ How it works", expanded=False):
        st.markdown("""
        **Hallucination Detection by Consistency Check**:
        
        **Key Difference - Sampling Strategy:**
        - **Main Response**: Generated deterministically (temperature = 0.0, beam search)
        - **N Samples**: Generated stochastically (temperature = 1.0, sampling enabled)
        - **Same Prompt**: Both use the exact same query/prompt
        
        **Process:**
        1. **Generate main response**: Create response deterministically (temp=0.0)
        2. **Generate N samples**: Create multiple stochastic responses (temp=1.0) 
        3. **Extract claims**: Intelligently split the main response into verifiable statements
        4. **Check consistency**: Verify if each claim is supported by the stochastic samples
        5. **Score**: Claims frequently contradicted indicate inconsistency (proxy for hallucination risk)
